 ...
 * Если клиенту отказали в выдаче кредита по причине плохой банковской истории - метод должен выводить в консоль
 * сообщение "Проблемы с банковской историей", если клиенту отказали по причине проблем с законом то ничего выводить
 * на экран не нужно.
 */
public class task426 {
    /*
    public interface BankWorker {
        boolean checkClientForCredit();
    }
    ...
    public boolean getCreditForClient(BankWorker bankWorker, BankClient bankClient)  {
        ...
        } catch (BadCreditHistoryException ignored) {
            System.out.println("Проблемы с банковской историей");
        } catch (ProblemWithLawException ignored) {
        }
        return false;
        // https://stackoverflow.com/questions/28659462/how-to-ignore-exceptions-in-java
    }
