...
//                    stringBuilder.append((i + 1) + ")" + textLines[i].substring(role.length() + 1) + "\n");
//                }
//            }
//        } почему-то ментору не понравилось

        for (String role : roles) {
            stringBuilder.append("\n").append(role).append(":\n");
            for (int i = 0; i < textLines.length; i++) {
                if (textLines[i].startsWith(role + ":")) {
                    stringBuilder.append(i + 1).append(") ").append(textLines[i].substring(role.length() + 2)).append("\n");
                }
            }
        }
